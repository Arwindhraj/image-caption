...
class Decoder(Model):
    def __init__(self, embed_dim, units, vocab_size):
        ...
        self.units=units
        self.attention = LocalAttention(self.units) #iniitalise your Attention model with units
        self.attentionglobal = GlobalAttention(self.units) #iniitalise your Attention model with units
    ...
    def call(self,x,features, hidden):
        local_context_vector, local_attention_weights = self.attention(features, hidden)
        global_context_vector, global_attention_weights = self.attentionglobal(features, hidden)
        combined_context_vector = tf.concat([local_context_vector, global_context_vector], axis=-1) #create your context vector & attention weights from attention model
        embed = self.embed(x) # embed your input to shape: (batch_size, 1, embedding_dim)
        embed = tf.concat([tf.expand_dims(combined_context_vector, 1), embed], axis = -1) # Concatenate your input with the context vector from attention layer. Shape: (batch_size, 1, embedding_dim + embedding_dim)
        ...
        return output, state, combined_attention_weights
