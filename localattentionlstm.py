...
class Decoder(Model):
    def __init__(self, embed_dim, units, vocab_size):
        ...
        self.units=units
        self.attentionlocal = LocalAttention(self.units) 
    ...
    def call(self,x,features, hidden,cell):
        local_context_vector, local_attention_weights = self.attentionlocal(features, hidden)
        embed = self.embed(x) 
        embed = tf.concat([tf.expand_dims(local_context_vector, 1), embed], axis = -1)
        # output,state = self.gru(embed) 
        output, state, cell = self.lstm(embed, initial_state=[hidden, cell])
        output = self.d1(output)
        output = tf.reshape(output, (-1, output.shape[2])) 
        output = self.d2(output) 
        return output, state, cell, local_attention_weights
...
hidden,cell = decoder.init_state(batch_size=sample_cap_batch.shape[0])
...
def train_step(img_tensor, target):
    ...
    with tf.GradientTape() as tape:
        ...
        for r in range(1, target.shape[1]) :
          predictions, hidden, _ ,_ = decoder(dec_input, encoder_op, hidden)
